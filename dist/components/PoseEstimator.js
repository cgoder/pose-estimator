...
export class PoseEstimator {
    ...
    async _setupCamera() {
        try {
            ...
            // 获取摄像头流
            const stream = await ErrorHandler.retry(() => navigator.mediaDevices.getUserMedia(CONFIG.CAMERA.CONSTRAINTS), 3, 1000);
            if (!stream) {
    ...
    async _loadModel() {
        try {
            ...
            this.detector = await modelCacheManager.getOrCreateModel(this.options.modelType, modelUrl, async () => {
                if (this.options.modelType === 'MoveNet') {
                    ...
                    return await poseDetection.createDetector(poseDetection.SupportedModels.PoseNet, {
                        quantBytes: 2,
    ...
    async _detectPoseInRealTime() {
        ...
        try {
            ...
            if (poses && poses.length > 0) {
                const pose = poses[0];
                // 应用One Euro Filter
                const filteredKeypoints = this.filterManager.filterPose(pose.keypoints, performance.now());
                // 绘制姿态
        ...
        keypoints.forEach((keypoint, index) => {
            if (keypoint && keypoint.score > CONFIG.UI.CONFIDENCE_THRESHOLD) {
                ...
                if (this.options.showKeypointLabels) {
                    ...
                    this.ctx.fillText(KEYPOINT_NAMES[index] || index.toString(), keypoint.x + 5, keypoint.y - 5);
                }
            }
        });
    }
    ...
    static async preloadModels() {
        ...
        const modelConfigs = [
            {
                ...
                createFn: () => poseDetection.createDetector(poseDetection.SupportedModels.MoveNet, {
                    modelType: poseDetection.movenet.modelType.SINGLEPOSE_LIGHTNING
                })
            },
            {
                ...
                createFn: () => poseDetection.createDetector(poseDetection.SupportedModels.PoseNet, {
                    quantBytes: 2,
                    architecture: 'MobileNetV1',
                    outputStride: 16,
                    inputResolution: { width: 353, height: 257 },
        ...
        const preloadPromises = modelConfigs.map(config => modelCacheManager.preloadModel(config.type, config.url, config.createFn));
        try {
...
//# sourceMappingURL=PoseEstimator.js.map