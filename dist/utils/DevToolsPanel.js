...
export class DevToolsPanel {
     ...
     * 添加面板
     */
    addPanel(id, title, icon, content) {
        const panel = {
            id,
            title,
            icon,
     ...
     */
    setupDragAndDrop() {
        const header = this.container?.querySelector('.dev-tools-header');
    ...
    minimize() {
        if (this.container) {
            const content = this.container.querySelector('.dev-tools-content');
    ...
    updateSystemStatus() {
        ...
        const uptime = Date.now() - window.startTime || 0;
        const statusHtml = `
