...
export class DependencyConfigValidator {
        ...
        if (tfVersionMatches) {
            ...
            if (foundVersion) {
                ...
                if (foundVersion !== this.expectedVersions.tensorflow) {
                    result.errors.push(`TensorFlow.js 版本不匹配: 期望 ${this.expectedVersions.tensorflow}, 实际 ${foundVersion}`);
                    result.isValid = false;
                }
            }
        }
        ...
        if (poseVersionMatches) {
            ...
            if (foundVersion) {
                ...
                if (foundVersion !== this.expectedVersions.poseDetection) {
                    result.errors.push(`Pose Detection 版本不匹配: 期望 ${this.expectedVersions.poseDetection}, 实际 ${foundVersion}`);
                    result.isValid = false;
                }
            }
        }
     ...
     * 获取修复建议
     */
    getFixSuggestions(result) {
        const suggestions = [];
        if (result.foundVersions.tensorflow && result.foundVersions.tensorflow !== this.expectedVersions.tensorflow) {
            suggestions.push(`将 TensorFlow.js 版本从 ${result.foundVersions.tensorflow} 更新到 ${this.expectedVersions.tensorflow}`);
        }
        if (result.foundVersions.poseDetection && result.foundVersions.poseDetection !== this.expectedVersions.poseDetection) {
            suggestions.push(`将 Pose Detection 版本从 ${result.foundVersions.poseDetection} 更新到 ${this.expectedVersions.poseDetection}`);
        }
...
//# sourceMappingURL=DependencyConfigValidator.js.map