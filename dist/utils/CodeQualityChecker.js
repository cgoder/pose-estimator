...
export class CodeQualityChecker {
    ...
    detectPerformanceIssues() {
        ...
        try {
            // 检查长任务
            const longTasks = this.performanceHistory.filter(entry => entry.duration > this.config.performanceThreshold);
            ...
            const slowResources = resourceTimings.filter((resource) => resource.duration > 2000);
    ...
    detectSecurityIssues() {
        ...
        try {
            ...
            // 检查潜在的数据暴露
            const globalVars = Object.keys(window).filter(key => key.includes('token') ||
                key.includes('key') ||
                key.includes('secret') ||
    ...
    detectCodeSmells() {
        ...
        try {
            // 检查全局变量过多
            const globalVarCount = Object.keys(window).filter(key => !key.startsWith('webkit') &&
                !key.startsWith('moz') &&
                !key.startsWith('ms') &&
                typeof window[key] !== 'function' ||
                key.includes('tf') ||
                key.includes('pose')).length;
    ...
    recordError(errorMessage) {
        if (!this.config.enableErrorPatternAnalysis)
            return;
     ...
     * 分类错误严重程度
     */
    categorizeErrorSeverity(pattern) {
...
//# sourceMappingURL=CodeQualityChecker.js.map