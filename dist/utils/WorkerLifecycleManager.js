...
export class WorkerLifecycleManager {
     ...
     * 创建并管理 Worker
     */
    async createWorker(workerId, workerScript, options = {}) {
        try {
            // 如果 Worker 已存在，先清理
     ...
     * 设置 Worker 事件监听器
     */
    setupWorkerEventListeners(workerId, worker) {
     ...
     * 发送消息到 Worker
     */
    async sendMessage(workerId, message, timeout = 30000) {
     ...
     * 更新平均响应时间
     */
    updateAverageResponseTime(workerId, responseTime) {
        const metrics = this.metrics.get(workerId);
        if (!metrics)
            return;
        ...
        for (const [workerId, health] of healthResults) {
            if (!health.isHealthy) {
                ...
                if (this.config.enableAutoRestart &&
                    health.issues.some(issue => issue.includes('错误状态') || issue.includes('长时间无活动'))) {
                    this.scheduleWorkerRestart(workerId);
                }
            }
        }
...
//# sourceMappingURL=WorkerLifecycleManager.js.map