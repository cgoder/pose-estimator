...
class PoseEstimationApp {
    ...
    async performEnvironmentChecks() {
        ...
        // 注意：根据架构设计，主线程不再检查 TensorFlow.js 依赖
        // TensorFlow.js 推理将完全在 Worker 中进行，主线程只负责 UI 和数据源管理
        console.log('✅ 环境检查通过');
    }
    /**
     * 初始化Canvas
    ...
    async changeModel(modelType) {
        try {
            ...
            uiManager.showError(`模型切换失败: ${errorMessage}`);
    ...
    async restart() {
        try {
            ...
            // 重新创建和启动
            this.createPoseEstimator();
            // 此时 poseEstimator 已经被重新创建，但 TypeScript 无法推断，所以使用类型断言
            const estimator = this.poseEstimator;
            ...
            uiManager.hideLoading();
            const errorMessage = error?.message || String(error);
            uiManager.showError(`重启失败: ${errorMessage}`);
    ...
    async clearCache() {
        try {
            ...
            uiManager.hideLoading();
            const errorMessage = error?.message || String(error);
            uiManager.showError(`清空缓存失败: ${errorMessage}`);
    ...
    async togglePause() {
        ...
        try {
            ...
            const errorMessage = error?.message || String(error);
            uiManager.showError(`操作失败: ${errorMessage}`);
    ...
    getErrorMessage(error) {
        ...
        if (errorObj?.name === 'CameraError') {
            return `摄像头错误: ${message}<br><br>解决方案:<br>• 确保已授权摄像头访问权限<br>• 检查摄像头是否被其他应用占用<br>• 尝试刷新页面重新授权`;
        }
        if (errorObj?.name === 'ModelError') {
            return `模型加载错误: ${message}<br><br>解决方案:<br>• 检查网络连接<br>• 清空浏览器缓存后重试<br>• 尝试切换到其他模型`;
        }
        if (errorObj?.name === 'StartupError') {
            return `启动错误: ${message}<br><br>解决方案:<br>• 确保使用HTTPS访问<br>• 检查浏览器兼容性<br>• 尝试刷新页面`;
        }
        if (message.includes('HTTPS') || message.includes('摄像头')) {
        ...
        return `未知错误: ${message}<br><br>请尝试刷新页面或联系技术支持`;
    }
    /**
     * 清理应用资源
     * @returns {Promise<void>}
...
async function main() {
    try {
        ...
        if (typeof uiManager !== 'undefined') {
            ...
            uiManager.hideLoading();
            const errorMessage = error?.message || String(error);
            uiManager.showError(`应用启动失败: ${errorMessage}<br><br>请检查控制台获取详细错误信息`, 0);
        }
    }
}
// 等待DOM加载完成后启动应用
