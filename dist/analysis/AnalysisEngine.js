...
export class FitnessAnalysisEngine {
    ...
    setExercise(config) {
        this.currentExercise = config.type;
        this.exerciseParameters = config.parameters;
        // 使用运动参数配置分析器
        if (this.exerciseParameters) {
            this.configureAnalyzersForExercise(config.type, this.exerciseParameters);
        }
        this.reset();
     ...
     * 为特定运动配置分析器
     */
    configureAnalyzersForExercise(exerciseType, parameters) {
        // 根据运动类型和参数配置分析器
        switch (exerciseType) {
            case 'squat':
    ...
    /**
      * 配置深蹲分析器
      */
    configureSquatAnalyzer(parameters) {
        const analyzer = {
            type: 'squat',
     ...
     * 配置俯卧撑分析器
     */
    configurePushupAnalyzer(parameters) {
        const analyzer = {
            type: 'pushup',
     ...
     * 配置平板支撑分析器
     */
    configurePlankAnalyzer(parameters) {
        const analyzer = {
            type: 'plank',
     ...
     * 配置跑步分析器
     */
    configureRunningAnalyzer(parameters) {
        const analyzer = {
            type: 'running',
     ...
     * 配置通用分析器
     */
    configureGeneralAnalyzer(parameters) {
        const analyzer = {
            type: 'general',
     ...
     * 计算运动指标
     */
    calculateMovementMetrics(poseResult) {
        const metrics = {};
     ...
     * 更新重复次数
     */
    updateRepetitionCount(poseResult) {
        if (!poseResult.poses.length)
            return;
        const pose = poseResult.poses[0];
        switch (this.currentExercise) {
     ...
     * 计算运动速度
     */
    calculateVelocity(currentPose, previousPose) {
        if (!currentPose.keypoints || !previousPose.keypoints)
            return 0;
        let totalMovement = 0;
        let validPoints = 0;
        for (const currentKp of currentPose.keypoints) {
            const previousKp = previousPose.keypoints.find((kp) => kp.name === currentKp.name);
            if (previousKp) {
                const distance = Math.sqrt(Math.pow(currentKp.x - previousKp.x, 2) +
     ...
     * 计算置信度
     */
    calculateConfidence(poseResult) {
        if (!poseResult.poses.length)
            return 0;
        const pose = poseResult.poses[0];
        if (!pose?.keypoints)
...
export class AnalysisEngineFactory {
    static create(type = 'fitness', eventBus) {
        switch (type) {
            case 'fitness':
