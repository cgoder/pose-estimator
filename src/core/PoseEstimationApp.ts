...
export class PoseEstimationApp {
   ...
   * 合并默认配置
   */
  private mergeWithDefaultConfig(config?: Partial<AppConfig>): AppConfig {
  ...
  async init(canvas?: HTMLCanvasElement): Promise<void> {
    try {
      ...
      this.emitAppEvent('error', undefined, error as Error);
      throw error;
    }
  }
   ...
   * @param options 选项
   */
  async setDataSource(type: 'camera' | 'videoFile' | 'imageFile', options?: any): Promise<void> {
    try {
      // 停止当前数据源
      if (this.dataSource) {
      ...
      switch (type) {
        case 'camera':
          ...
          break;
        case 'videoFile':
          if (!options?.file) throw new Error('视频文件数据源需要提供文件');
          this.dataSource = dataSourceFactory.createVideoFileSource(options.file, options?.config);
          break;
        case 'imageFile':
          if (!options?.files) throw new Error('图像文件数据源需要提供文件数组');
   ...
   * 绑定数据源事件
   */
  private bindDataSourceEvents(): void {
    if (!this.dataSource) return;

    this.dataSource.on('frame', (imageData: any) => {
      this.processFrame(imageData);
    });

    this.dataSource.on('error', (error: any) => {
   ...
   * 更新性能指标
   */
  private updatePerformanceMetrics(inferenceTime: number, totalTime: number): void {
    const currentState = stateManager.getState();
    stateManager.setState({
      performance: {
   ...
   * 发送应用事件
   */
  private emitAppEvent(type: AppEventType, data?: any, error?: Error): void {
  ...
  private async initializeWorkerManager(): Promise<void> {
    try {
      ...
      this.workerManager = await WorkerManagerFactory.create(
        this.config?.inference.enableWorker ?? true
      );
...
export function createPoseEstimationApp(config?: Partial<AppConfig>): PoseEstimationApp {
