...
export class LocalDependencyManager {
  private config: LocalDependencyConfig;
  ...
  async loadAllDependencies(): Promise<void> {
    const loadPromises = Array.from(this.dependencies.keys()).map(key => 
   ...
   * 执行实际的加载操作
   */
  private async performLoad(dependency: DependencyInfo): Promise<void> {
   ...
   * 获取加载方法列表
   */
  private getLoadMethods(dependency: DependencyInfo): Array<{type: string, url: string}> {
    const methods: Array<{type: string, url: string}> = [];
   ...
   * 尝试加载方法
   */
  private async tryLoadMethod(method: {type: string, url: string}): Promise<void> {
...
`;
   ...
   * 创建本地依赖文件
   */
  async createLocalDependencies(): Promise<void> {
   ...
   * 下载并保存本地文件
   */
  private async downloadAndSaveLocal(dependency: DependencyInfo): Promise<void> {
    try {
