...
export class ConfigManager {
   ...
   * 设置配置值
   */
  set(key: string, value: any, validate = true): void {
   ...
   * 导出配置
   */
  export(includeDefaults = false): Record<string, any> {
    if (includeDefaults) {
   ...
   * 导入配置
   */
  import(configs: Record<string, any>, validate = true): void {
    if (validate) {
      // 先验证所有配置
  ...
  generateDocs(): string {
    ...
    for (const key of Object.keys(this.schema)) {
      ...
      const category: string = parts[0] || 'general'; // 明确类型注解，确保不为空
    ...
    for (const [category, keys] of categories) {
      ...
      for (const key of keys) {
        const schema = this.schema[key];
        if (!schema) continue; // 跳过不存在的 schema
