...
export class PoseEstimator {
    ...
    private options: any;
    public filterManager: OneEuroFilterManager;
    private stats: {
        frameCount: number;
     ...
     */
    async _setupCamera(): Promise<void> {
        try {
            console.log('📷 正在设置摄像头...');
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                this.video!.addEventListener('error', (error: any) => {
                    clearTimeout(timeout);
     ...
     * 姿态检测循环
     */
    async _detectPoseInRealTime(): Promise<void> {
        ...
        keypoints.forEach((keypoint, index) => {
            if (keypoint && keypoint.score > CONFIG.UI.CONFIDENCE_THRESHOLD) {
                ...
                if (this.options.showKeypointLabels) {
                    ...
                    this.ctx!.fillText(
                        KEYPOINT_NAMES[index] || index.toString(),
                        keypoint.x + 5,
                        keypoint.y - 5
                    );
                }
