...
export class UIManager {
    ...
    private filterPanelElement: HTMLElement | null;
    private modelPanelElement: HTMLElement | null;
    private isInitialized: boolean;
    private isMenuOpen: boolean;
    private menuPanel: HTMLElement | null;
    private mainButton: HTMLElement | null;
    private activeSubPanels: Set<string>;
    private panelCallbacks: any;
    ...
    createControlsElement(): void {
        ...
        menuItems.forEach(item => {
            ...
            item.addEventListener('mouseenter', () => {
                ...
                (item as HTMLElement).style.borderColor = 'rgba(52, 152, 219, 0.5)';
            ...
            item.addEventListener('click', (e) => {
                ...
                if (panelType) {
                    this.toggleSubPanel(panelType, item as HTMLElement);
                }
            });
        });
        ...
        document.addEventListener('click', (e) => {
            if (this.controlsElement && e.target && 
                !this.controlsElement.contains(e.target as Node) && 
                !this.isClickOnSubPanel(e.target as HTMLElement) && 
                this.isMenuOpen) {
                this.toggleControlMenu();
            }
        });
     ...
     */
    hideSubPanel(panelType: string, immediate: boolean = false): void {
        switch (panelType) {
            case 'model':
    ...
    hideOtherSubPanels(excludePanelType: string): void {
        ...
        if (this.controlsElement) {
            ...
            menuItems.forEach(item => {
                const panelType = item.getAttribute('data-panel');
                if (panelType !== excludePanelType) {
                    this.updateMenuItemState(item as HTMLElement, false);
                }
            });
        }
    }
    ...
    updateMenuItemState(menuItem: HTMLElement, isActive: boolean): void {
        const arrow = menuItem.querySelector('.menu-arrow') as HTMLElement;
    ...
    createStatusElement(): void {
        ...
        // 绑定最小化/展开事件
        const toggleBtn = titleBar.querySelector('#toggle-status-btn') as HTMLButtonElement;
    ...
    createModelPanelElement(): void {
        ...
        // 绑定折叠/展开事件
        const toggleBtn = titleBar.querySelector('#toggle-model-panel-btn') as HTMLButtonElement;
        let isMinimized = false;
        toggleBtn!.addEventListener('click', () => {
            isMinimized = !isMinimized;
            contentArea.style.display = isMinimized ? 'none' : 'block';
    ...
    createFilterPanelElement(): void {
        ...
        // 绑定折叠/展开事件
        const toggleBtn = titleBar.querySelector('#toggle-filter-panel-btn') as HTMLButtonElement;
        let isMinimized = false;
        toggleBtn!.addEventListener('click', () => {
            isMinimized = !isMinimized;
            contentArea.style.display = isMinimized ? 'none' : 'block';
     ...
     */
    showLoading(message: string = '正在加载...', progress: string = ''): void {
        if (!this.loadingElement) this.createLoadingElement();
        
        const textElement = this.loadingElement!.querySelector('#loading-text') as HTMLElement;
        const progressElement = this.loadingElement!.querySelector('#loading-progress') as HTMLElement;
    ...
    showError(message: string, duration: number = 5000): void {
        if (!this.errorElement) this.createErrorElement();
     ...
     */
    showControls(): void {
        if (!this.controlsElement) this.createControlsElement();
    ...
    toggleControlMenu(): void {
        if (!this.controlsElement || !this.menuPanel || !this.mainButton) return;
    ...
    updateStatus(status: any): void {
        if (!this.statusElement) this.createStatusElement();
...
内存使用: ${tfMem.numBytes}MB`;
        ...
        const contentArea = this.statusElement!.querySelector('#status-content') as HTMLElement || this.statusElement!;
    ...
    _adjustModelPanelSize(): void {
        ...
        const contentArea = this.modelPanelElement.querySelector('#model-panel-content') as HTMLElement;
    ...
    _adjustFilterPanelSize(): void {
        ...
        const contentArea = this.filterPanelElement.querySelector('#filter-panel-content') as HTMLElement;
        if (!contentArea) return;
    ...
    _adjustStatusElementSize(): void {
        ...
        const contentArea = this.statusElement.querySelector('#status-content') as HTMLElement;
        if (!contentArea) return;
     ...
     * @param {boolean} immediate - 是否立即隐藏（跳过动画）
     */
    hideStatus(immediate: boolean = false): void {
        if (this.statusElement) {
            if (immediate) {
                // 立即隐藏，跳过动画
     ...
     */
    showModelPanel(): void {
        if (!this.modelPanelElement) this.createModelPanelElement();
     ...
     * @param {boolean} immediate - 是否立即隐藏（跳过动画）
     */
    hideModelPanel(immediate: boolean = false): void {
        if (this.modelPanelElement) {
            if (immediate) {
                // 立即隐藏，跳过动画
    ...
    showFilterPanel(): void {
        if (!this.filterPanelElement) this.createFilterPanelElement();
        ...
        const enableCheckbox = this.filterPanelElement!.querySelector('#enable-filter-checkbox') as HTMLInputElement;
     ...
     * @param {boolean} immediate - 是否立即隐藏（跳过动画）
     */
    hideFilterPanel(immediate: boolean = false): void {
        if (this.filterPanelElement) {
            if (immediate) {
                // 立即隐藏，跳过动画
    ...
    bindModelPanelEvents(callbacks: any = {}): void {
        ...
        // 模型选择
        const modelSelect = this.modelPanelElement.querySelector('#model-select') as HTMLSelectElement;
        if (modelSelect && callbacks.onModelChange) {
            modelSelect.addEventListener('change', (e) => {
                callbacks.onModelChange((e.target as HTMLSelectElement).value);
            });
        }
        ...
        Object.entries(checkboxes).forEach(([selector, callback]) => {
            const element = this.modelPanelElement!.querySelector(selector) as HTMLInputElement;
            if (element && callback) {
        ...
        // 重启检测按钮
        const restartBtn = this.modelPanelElement.querySelector('#restart-model-btn') as HTMLButtonElement;
        ...
        // 清空缓存按钮
        const clearCacheBtn = this.modelPanelElement.querySelector('#clear-cache-model-btn') as HTMLButtonElement;
    ...
    bindFilterPanelEvents(callbacks: any = {}): void {
        ...
        // 预设选择
        const presetSelect = this.filterPanelElement.querySelector('#filter-preset') as HTMLSelectElement;
        if (presetSelect && callbacks.onPresetChange) {
            presetSelect.addEventListener('change', (e) => {
                callbacks.onPresetChange((e.target as HTMLSelectElement).value);
            });
        }
        ...
        Object.entries(sliders).forEach(([selector, config]) => {
            const slider = this.filterPanelElement!.querySelector(selector) as HTMLInputElement;
            const valueElement = this.filterPanelElement!.querySelector(config.valueElement) as HTMLElement;
        ...
        // 重置按钮
        const resetBtn = this.filterPanelElement!.querySelector('#reset-filter-params') as HTMLButtonElement;
        if (resetBtn && callbacks.onReset) {
            resetBtn.addEventListener('click', callbacks.onReset);
        }

        // 应用按钮
        const applyBtn = this.filterPanelElement!.querySelector('#apply-filter-config') as HTMLButtonElement;
    ...
    updateFilterPanelParams(params: any): void {
        ...
        // 更新滑块值和显示
        const updates = {
            '#frequency-slider': { value: params.frequency, display: '#frequency-value', format: (v: number) => Math.round(v) },
            '#minCutoff-slider': { value: params.minCutoff, display: '#minCutoff-value', format: (v: number) => v.toFixed(3) },
            '#beta-slider': { value: params.beta, display: '#beta-value', format: (v: number) => v.toFixed(3) },
            '#dCutoff-slider': { value: params.dCutoff, display: '#dCutoff-value', format: (v: number) => v.toFixed(3) }
        };

        Object.entries(updates).forEach(([selector, config]) => {
            const slider = this.filterPanelElement!.querySelector(selector) as HTMLInputElement;
            const display = this.filterPanelElement!.querySelector(config.display) as HTMLElement;
    ...
    updateFilterControlsState(enabled: boolean): void {
        ...
        const filterControls = this.filterPanelElement!.querySelector('#filter-controls') as HTMLElement;
        if (filterControls) {
            // 控制整个控件容器的透明度和指针事件
            ...
            // 禁用所有输入控件
            const inputs = filterControls.querySelectorAll('input, select, button') as NodeListOf<HTMLInputElement | HTMLSelectElement | HTMLButtonElement>;
    ...
    updateControlsState(options: any): void {
        if (!this.controlsElement || !this.menuPanel) return;
        ...
        Object.entries(panelStates).forEach(([panelType, isEnabled]) => {
            const menuItem = this.menuPanel!.querySelector(`[data-panel="${panelType}"]`) as HTMLElement;
