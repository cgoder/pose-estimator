...
export class OneEuroFilterManager {
    private filterParams: FilterConfig;
     ...
     */
    filterPose(keypoints: Keypoint[], timestamp: number = Date.now()): Keypoint[] {
    ...
    updateParameters(newParams: Partial<FilterConfig>): void {
        ...
        const hasChanged = Object.keys(this.filterParams).some(
            (key: string) => (this.filterParams as any)[key] !== (oldParams as any)[key]
        );
     ...
     * @private
     */
    private updateExistingFilters(): void {
     ...
     */
    static getRecommendedParameters(scenario: string = 'default'): PresetConfig | null {
