...
内存使用: ${tfMem.numBytes}MB`;
    ...
    _adjustModelPanelSize() {
        ...
        // 临时设置为自动宽度以测量内容
        const originalWidth = this.modelPanelElement.style.width;
        const originalHeight = this.modelPanelElement.style.height;
    ...
    _adjustFilterPanelSize() {
        ...
        // 临时设置为自动宽度以测量内容
        const originalWidth = this.filterPanelElement.style.width;
        const originalHeight = this.filterPanelElement.style.height;
    ...
    _adjustStatusElementSize() {
        ...
        // 临时设置为自动宽度以测量内容
        const originalWidth = this.statusElement.style.width;
        const originalHeight = this.statusElement.style.height;
    ...
    updateFilterPanelParams(params) {
        ...
        Object.entries(updates).forEach(([selector, config]) => {
            ...
            if (display && config.value !== undefined) {
                display.textContent = config.format(config.value);
            }
        });
    }
